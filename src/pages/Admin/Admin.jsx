    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h3 className="text-lg font-bold text-[#60804F]">{title}</h3>
...
function ConfirmarEliminar({ nombre, onConfirmar, onCancelar }) {
  return (
    <Modal title="Confirmar eliminación" onClose={onCancelar}>
      <p className="text-slate-600 mb-6">¿Estás seguro que querés eliminar <strong>{nombre}</strong>? Esta acción no se puede deshacer.</p>
      <div className="flex gap-3">
        <button onClick={onCancelar} className="flex-1 py-2 rounded-md border border-slate-200 text-sm text-slate-600 hover:bg-slate-50 transition-colors">Cancelar</button>
        <button onClick={onConfirmar} className="flex-1 py-2 rounded-md bg-red-500 text-white text-sm font-semibold hover:bg-red-600 transition-colors">Eliminar</button>
      </div>
    </Modal>
  )
}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold text-[#60804F]">Productos ({productos.length})</h2>
        <button onClick={abrirCrear} className="bg-[#8fbc6a] text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-[#60804F] transition-colors">+ Nuevo producto</button>
      </div>
      ...
      {(modal === 'crear' || modal === 'editar') && (
        <Modal title={modal === 'crear' ? 'Nuevo producto' : 'Editar producto'} onClose={() => setModal(null)}>
          <div className="space-y-3">
            ...
            <Campo label="Badge"><input name="badge" value={form.badge} onChange={handleChange} className={inputClass} placeholder="Nuevo, Oferta..." /></Campo>
            <Campo label="Stock">
            ...
            <div className="flex gap-3 pt-2">
              <button onClick={() => setModal(null)} className="flex-1 py-2 rounded-md border border-slate-200 text-sm text-slate-600 hover:bg-slate-50 transition-colors">Cancelar</button>
              <button onClick={guardar} disabled={guardando} className="flex-1 py-2 rounded-md bg-[#8fbc6a] text-white text-sm font-semibold hover:bg-[#60804F] disabled:opacity-50 transition-colors">
...
function TabUsuarios() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold text-[#60804F]">Usuarios ({usuarios.length})</h2>
        <button onClick={abrirCrear} className="bg-[#8fbc6a] text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-[#60804F] transition-colors">+ Nuevo usuario</button>
      </div>
...
function TabPedidos() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold text-[#60804F]">Pedidos ({pedidos.length})</h2>
                  ...
                  <td className="px-3 py-2">
                    {p.estado === 'cancelado' ? (
                      ...
                      <select
                        ...
                        className="text-xs border border-[#60804F]/30 rounded-md px-2 py-1 text-slate-700 focus:outline-none focus:ring-1 focus:ring-[#60804F] disabled:opacity-50"
                      >
      ...
      {pedidoDetalle && (
        <Modal title={`Pedido #${pedidoDetalle.nroOrden}`} onClose={() => setPedidoDetalle(null)}>
          <div className="space-y-3 text-sm">
            <div>
              <p className="text-[#8fbc6a] font-medium mb-1">Datos de envío</p>
              <p className="text-slate-700 capitalize">{pedidoDetalle.datosEnvio?.nombre}</p>
              <p className="text-slate-500">{pedidoDetalle.datosEnvio?.email}</p>
              <p className="text-slate-500">{pedidoDetalle.datosEnvio?.direccion}, {pedidoDetalle.datosEnvio?.ciudad}, {pedidoDetalle.datosEnvio?.provincia} ({pedidoDetalle.datosEnvio?.cp})</p>
              <p className="text-slate-500">Modalidad: {pedidoDetalle.datosEnvio?.modalidad === 'retiro' ? 'Retiro en local' : 'Envío a domicilio'}</p>
            </div>
            <div>
              <p className="text-[#8fbc6a] font-medium mb-1">Productos</p>
...
function TabResenas() {
  ...
  const abrirCrear = () => {
    setForm({ nombre: '', mascota: '', avatar: '', texto: '', producto: '' })
    setError('')
    setModal('crear')
  }
  ...
  const abrirEditar = (r) => {
    setSeleccionado(r)
    setForm({ nombre: r.nombre, mascota: r.mascota, avatar: r.avatar ?? '', texto: r.texto, producto: r.producto ?? '' })
    setError('')
    setModal('editar')
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold text-[#60804F]">Reseñas ({resenas.length})</h2>
        <button onClick={abrirCrear} className="bg-[#8fbc6a] text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-[#60804F] transition-colors">+ Nueva reseña</button>
      </div>
...
export default function Admin() {
  ...
  return (
    <div className="min-h-[85vh] bg-white px-4 py-8">
      <div className="max-w-6xl mx-auto">

        <div className="flex items-center gap-3 mb-6">
          <div>
            <h1 className="text-2xl font-bold text-[#60804F]">Panel de Administración</h1>
            <p className="text-sm text-slate-400">Gestioná productos, usuarios y pedidos</p>
          </div>
        </div>
